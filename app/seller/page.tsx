...
async function read(response: Response) {
  ...
  if (!response.ok)
    throw new Error(value.error || 'That request could not be completed.');
  return value;
}
...
export default function SellerStudio() {
  ...
  const load = useCallback(async () => {
    try {
      if (!(await cognitoToken())) {
        window.location.href = '/sign-in?next=/seller';
        return;
      }
      const accountResponse = await awsApi('/account'),
        accountData = await accountResponse.json();
      if (
        !accountResponse.ok ||
        !accountData.account?.terms_accepted ||
    ...
    try {
      await read(
        await awsApi('/seller', {
          ...
          body: JSON.stringify({
            ...payload,
            servicePrice: Number(payload.servicePrice),
            availabilityDays: form.getAll('availabilityDays').join(','),
          }),
        }),
      );
      setNotice('Your seller profile is live.');
      await load();
    } catch (cause) {
      setError(
  ...
  async function addService(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setError(
        cause instanceof Error ? cause.message : 'Unable to add service.',
      );
    } finally {
      setBusy(false);
    }
  ...
  async function upload(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (file.size > maximum)
        throw new Error(
          `That ${file.type.startsWith('video/') ? 'video' : 'image'} is too large.`,
        );
      const created = await read(
        await awsApi('/media/upload-url', {
          method: 'POST',
      ...
      event.currentTarget.reset();
      setNotice('Portfolio upload complete.');
      await load();
    } catch (cause) {
      setError(
    ...
    <main className="page-shell">
      ...
      <div className="panel-grid">
        <section className="panel">
          ...
          <form className="form-grid" onSubmit={submitProfile}>
            ...
            <div className="field">
              ...
              <select
                name="specialty"
                defaultValue={profile?.specialty ?? 'Braids'}
              >
            ...
            <div className="field">
              <label>Starting price (ZAR)</label>
              <input
                name="servicePrice"
                type="number"
                min="0"
                required
                defaultValue={profile?.service_price ?? 350}
              />
            </div>
            <div className="field full">
              ...
              <input
                ...
                required
                defaultValue={profile?.featured_service}
              />
            </div>
            <div className="field full">
            ...
            <fieldset className="field full availability-field">
              <legend>Days you accept bookings</legend>
        ...
        <section className="panel">
          ...
          {!profile ? (
            <div className="empty">Publish your profile first.</div>
          ) : (
            <>
