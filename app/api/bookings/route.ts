...
function isCalendarDate(date: string) { const value = new Date(`${date}T12:00:00Z`); return /^\d{4}-\d{2}-\d{2}$/.test(date) && !Number.isNaN(value.getTime()) && value.toISOString().slice(0, 10) === date; }
...
export async function POST(request: Request) {
  ...
  const isDate = isCalendarDate(appointmentDate);
  const available = seller?.availability_days.split(',').includes(weekday(appointmentDate));
  if (!seller || seller.user_id === user.userId || !customerName || !customerPhone || !isDate || appointmentDate < southAfricaToday() || !available || !['09:00', '11:00', '13:00', '15:00'].includes(appointmentTime)) return NextResponse.json({ error: 'Choose an available future date and time, then try again.' }, { status: 400 });
