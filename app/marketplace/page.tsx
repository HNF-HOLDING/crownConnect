...
export default async function Marketplace({ searchParams }: { searchParams: Promise<{ q?: string; city?: string; specialty?: string; maxPrice?: string }> }) {
  ...
  return <>
    <header className="site-header"><a className="brand" href="/"><span aria-hidden>♛</span> CrownConnect</a><nav aria-label="Primary navigation"><Link href="/">About CrownConnect</Link><Link href="/marketplace">Marketplace</Link></nav>{user ? <AccountMenu role={account?.primary_role} /> : <Link className="button secondary" href="/welcome">Create account</Link>}</header>
